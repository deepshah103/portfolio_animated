...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Keep older portfolio links working after moving the 3D experience
    // from the root route to the dedicated /explore route.
    if (experience === '3d') {
      const start = query.get('start');
      const target = `${assetPath('/explore')}${start ? `?start=${encodeURIComponent(start)}` : ''}`;
      ...
      <div className="relative z-10 mx-auto flex min-h-[100dvh] max-w-6xl flex-col px-6 py-8 md:px-10">
        ...
        <section className="flex flex-1 items-center justify-center py-12">
          <div className={`w-full max-w-5xl text-center transition-all duration-700 ${show ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
            ...
            <h1 className="mt-5 text-4xl font-semibold tracking-tight md:text-6xl">How would you like to explore?</h1>
            <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-300 md:text-lg">
              I build software, machine-learning systems and generative AI products. Choose an interactive walkthrough or jump straight into the work.
            </p>

            <div className="mt-10 grid gap-5 md:grid-cols-2">
              <Link href="/explore" className="group relative overflow-hidden rounded-3xl border border-cyan-300/20 bg-white/[0.06] p-7 text-left shadow-2xl backdrop-blur-xl transition hover:-translate-y-1 hover:border-cyan-300/50 hover:bg-cyan-300/[0.08]">
                ...
                <div className="relative">
                  ...
                  <p className="mt-3 min-h-14 text-sm leading-6 text-slate-300">Walk through my virtual workspace, discover projects and interact with the environment.</p>
                  <span className="mt-6 inline-flex rounded-xl bg-cyan-400 px-4 py-2.5 text-sm font-bold text-slate-950 transition group-hover:bg-cyan-300">Enter the workspace →</span>
                </div>
              </Link>

              <Link href="/flat" className="group relative overflow-hidden rounded-3xl border border-indigo-300/20 bg-white/[0.06] p-7 text-left shadow-2xl backdrop-blur-xl transition hover:-translate-y-1 hover:border-indigo-300/50 hover:bg-indigo-300/[0.08]">
                ...
                <div className="relative">
                  ...
                  <p className="mt-3 min-h-14 text-sm leading-6 text-slate-300">Browse my projects, experience, skills and contact information in a traditional portfolio.</p>
                  <span className="mt-6 inline-flex rounded-xl border border-white/15 bg-white/10 px-4 py-2.5 text-sm font-bold text-white transition group-hover:bg-white/15">Browse portfolio →</span>
                </div>
              </Link>
            </div>

            <div className="mt-9 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-xs text-slate-400">
              <span>Software Engineering</span><span>·</span><span>Data Science</span><span>·</span><span>Generative AI</span>
            </div>
          </div>
        </section>
